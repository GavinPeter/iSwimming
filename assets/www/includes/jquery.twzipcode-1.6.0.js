...
;(function ($, window, document, undefined) {
    ...
    'use strict';
	var area = {
		'北區': ['台北市','新北市','基隆市','桃園縣','新竹縣','新竹市'],
		'中區': ['苗栗縣','台中市','彰化縣','南投縣','雲林縣'],
		'南區': ['嘉義縣','嘉義市','台南市','高雄市','屏東縣'],
		'東區': ['宜蘭縣','花蓮縣','台東縣'],
		'離島': ['澎湖縣','台東縣','金門縣', '連江縣']
		 };
    // Zipcode JSON data
	...
	/*			$.each( area, function( key, value ) {
				...
				wrap.area.selectmenu(); 
				wrap.county.selectmenu(); 
				wrap.district.selectmenu(); 
				wrap.area.selectmenu("refresh");
				wrap.county.selectmenu("refresh");
				wrap.district.selectmenu("refresh");*/
				//wrap.county.show();
				//wrap.district.show();
                break;
            }
        ...
        bindings: function () {
            ...
            var self = this,
                ...
                wrap = self.wrap;
			//area
            wrap.area.on('change', function () {
                var val = $(this).val(),
                    tpl = [];
                wrap.county.empty().html('<option value="">縣市</option>');
                
				if (val!='M') {
					area[val].forEach(function(county) {
                            tpl.push('<option value="');
                            tpl.push(county);
                            tpl.push('">');
                            tpl.push(county);
					...
					$(tpl.join('')).appendTo(wrap.county);
					wrap.county.selectmenu("refresh");
					wrap.district.empty().html('<option value="">鄉鎮市區</option>');
					wrap.district.selectmenu("refresh");
					//self.reset('district');
                    //wrap.county.append(tpl.join('')).trigger('change');
                } else {
                    wrap.area.find('option:first').prop('selected', true);
                    self.reset();

                }
                // callback binding
                if ('function' === typeof opts.onAreaSelect) {
                    opts.onAreaSelect.call(this, wrap.area);
                }
            });
            // county
        ...
        init: function () {
			...
			*/
            this.wrap = {
				area: container.find('select[name="' + opts.areaName + '"]:first'),
